...
export const Dashboard: React.FC<DashboardProps> = ({ 
  ...
  const parseHistoryReason = (reason: string) => { 
      let resi = '-'; let ecommerce = '-'; let customer = '-'; let keterangan = reason || ''; 
  ...
  return (
    <div className="bg-gray-900 min-h-screen pb-24 font-sans text-gray-100">
      ...
      {/* --- HISTORY LOGS MODAL (GLOBAL - VERSI TABEL) --- */}
      {showHistoryDetail && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm animate-in fade-in">
             <div className="bg-gray-800 rounded-2xl w-full max-w-4xl h-[85vh] flex flex-col border border-gray-700 shadow-2xl m-4">
                         ...
                         <div className="overflow-x-auto rounded-lg border border-gray-700">
                             <table className="w-full text-left border-collapse text-sm">
                                <thead className="bg-gray-800 text-gray-400 uppercase text-xs font-bold">
                                    <tr>
                                        <th className="px-4 py-3 border-b border-gray-700 whitespace-nowrap">Tanggal</th>
                                        <th className="px-4 py-3 border-b border-gray-700">Barang</th>
                                        <th className="px-4 py-3 border-b border-gray-700">Tipe</th>
                                        <th className="px-4 py-3 border-b border-gray-700 text-right">Jumlah</th>
                                        <th className="px-4 py-3 border-b border-gray-700">Keterangan</th>
                                    </tr>
                                </thead>
                                        ...
                                        return (
                                            <tr key={h.id} className="hover:bg-gray-800/50 transition-colors">
                                                <td className="px-4 py-3 whitespace-nowrap text-gray-400 text-xs">
                                                    {new Date(h.timestamp || 0).toLocaleDateString('id-ID')} <br/>
                                                    <span className="text-[10px] opacity-70">{new Date(h.timestamp || 0).toLocaleTimeString('id-ID')}</span>
                                                </td>
                                                <td className="px-4 py-3">
                                                    <div className="font-medium text-gray-200">{h.name}</div>
                                                    <div className="text-[10px] text-gray-500 font-mono">{h.partNumber}</div>
                                                </td>
                                                <td className="px-4 py-3">
                                                    <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase border ${h.type === 'in' ? 'bg-green-900/30 text-green-400 border-green-800' : 'bg-red-900/30 text-red-400 border-red-800'}`}>
                                                        {h.type === 'in' ? 'Masuk' : 'Keluar'}
                                                    </span>
                                                </td>
                                                <td className={`px-4 py-3 text-right font-bold ${h.type === 'in' ? 'text-green-400' : 'text-red-400'}`}>
                                                    {h.type === 'in' ? '+' : '-'}{h.quantity}
                                                </td>
                                                <td className="px-4 py-3 text-xs text-gray-300">
                                                    <div className="font-medium">{keterangan}</div>
                                                    <div className="flex flex-wrap gap-1 mt-1">
                                                        {resi !== '-' && <span className="bg-gray-700 px-1.5 py-0.5 rounded text-[10px] text-blue-300 border border-gray-600">{resi}</span>}
                                                        {customer !== '-' && <span className="bg-gray-700 px-1.5 py-0.5 rounded text-[10px] text-gray-400 border border-gray-600">{customer}</span>}
                                                    </div>
                                                </td>
                                            </tr>
                                        )
      ...
      {/* --- HISTORY PER ITEM MODAL (VERSI TABEL) --- */}
      {selectedItemHistory && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm animate-in fade-in">
              <div className="bg-gray-800 rounded-2xl w-full max-w-2xl max-h-[85vh] flex flex-col border border-gray-700 shadow-2xl m-4 overflow-hidden">
                   ...
                   <div className="flex-1 overflow-auto p-0 bg-gray-900/30">
                       {loadingItemHistory ? ( <div className="flex justify-center py-8"><Loader2 className="animate-spin text-blue-500" size={24}/></div> ) : filteredItemHistory.length === 0 ? ( <div className="text-center py-8 text-gray-500 text-xs">Belum ada riwayat transaksi.</div> ) : (
                           <table className="w-full text-left border-collapse text-xs">
                               <thead className="bg-gray-800 text-gray-400 font-bold sticky top-0 z-10">
                                   <tr>
                                       <th className="px-4 py-2 border-b border-gray-700">Tanggal</th>
                                       <th className="px-4 py-2 border-b border-gray-700">Tipe</th>
                                       <th className="px-4 py-2 border-b border-gray-700 text-right">Qty</th>
                                       <th className="px-4 py-2 border-b border-gray-700">Keterangan</th>
                                   </tr>
                               </thead>
                                       ...
                                       return (
                                           <tr key={idx} className="hover:bg-gray-800/50">
                                               <td className="px-4 py-2 text-gray-400 whitespace-nowrap">
                                                   {new Date(h.timestamp || 0).toLocaleDateString('id-ID')} <span className="text-[9px] opacity-60 ml-1">{new Date(h.timestamp || 0).toLocaleTimeString('id-ID', {hour:'2-digit', minute:'2-digit'})}</span>
                                               </td>
                                               <td className="px-4 py-2">
                                                   <span className={`px-1.5 py-0.5 rounded text-[9px] font-bold uppercase ${h.type === 'in' ? 'bg-green-900/30 text-green-400 border border-green-800' : 'bg-red-900/30 text-red-400 border border-red-800'}`}>
                                                       {h.type === 'in' ? 'IN' : 'OUT'}
                                                   </span>
                                               </td>
                                               <td className={`px-4 py-2 text-right font-bold ${h.type === 'in' ? 'text-green-400' : 'text-red-400'}`}>
                                                   {h.type === 'in' ? '+' : '-'}{h.quantity}
                                               </td>
                                               <td className="px-4 py-2 text-gray-300">
                                                   <div className="font-medium">{keterangan}</div>
                                                   <div className="flex flex-wrap gap-1 mt-0.5">
                                                       {resi !== '-' && <span className="text-[9px] bg-gray-700 px-1 rounded text-blue-300">{resi}</span>}
                                                       {customer !== '-' && <span className="text-[9px] bg-gray-700 px-1 rounded text-gray-400">{customer}</span>}
                                                   </div>
                                               </td>
                                           </tr>
                                       );
