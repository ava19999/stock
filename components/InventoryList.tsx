...
import { Package, History, MapPin, Edit, Trash2, ChevronLeft, ChevronRight, Loader2 } from 'lucide-react';
        ...
        <div className="flex flex-col gap-3">
          {items.map(item => (
            <div key={item.id} className={`rounded-xl p-3 border shadow-none flex items-center gap-3 ${getItemCardStyle(item.quantity)}`}>
              <div className="w-16 h-16 bg-gray-700 rounded-lg overflow-hidden flex-shrink-0 cursor-pointer relative" onClick={() => onShowHistory(item)}>{item.imageUrl ? <img src={item.imageUrl} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-gray-600"><Package size={20}/></div>}</div>
