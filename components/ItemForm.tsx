...
import { Camera, Upload, Sparkles, X, Save, Loader2, Box, ChevronDown, Check, Crown, Tag } from 'lucide-react';
...
export const ItemForm: React.FC<ItemFormProps> = ({ initialData, onSubmit, onCancel }) => {
  ...
  // State untuk kontrol Menu Harga
  ...
  // Handler khusus untuk input harga dinamis
          ...
          <div className="w-full lg:w-2/3 space-y-5">
            ...
            <div className="bg-gray-50 p-4 rounded-xl border border-gray-100">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    {/* 2. HARGA JUAL (DINAMIS DENGAN POPUP) */}
                    <div className="relative">
                        <div className="flex justify-between items-center mb-1">
                            <label className={`block text-xs font-bold uppercase transition-colors flex items-center gap-1 ${activePriceType === 'retail' ? 'text-blue-600' : 'text-purple-600'}`}>
                                {activePriceType === 'retail' ? 'Harga Eceran (Umum)' : 'Harga King Fano'}
                            </label>
                            
                            {/* TOMBOL PEMICU POPUP */}
                            <button 
                                type="button" 
                                onClick={(e) => { e.stopPropagation(); setShowPriceMenu(!showPriceMenu); }}
                                className="text-[10px] flex items-center gap-1 bg-white border border-gray-300 px-2 py-0.5 rounded shadow-sm hover:bg-gray-50 text-gray-600 font-medium transition-all active:scale-95"
                            >
                                Ganti Tipe <ChevronDown size={10}/>
                            </button>
                        </div>
                        ...
                        {showPriceMenu && (
                            <div className="absolute right-0 top-7 z-20 w-48 bg-white rounded-lg shadow-xl border border-gray-200 animate-in zoom-in-95 duration-100 overflow-hidden">
                                <div className="p-1">
                                    ...
                                    <button 
                                        type="button"
                                        onClick={() => { setActivePriceType('retail'); setShowPriceMenu(false); }}
                                        className={`w-full text-left px-3 py-2 text-xs font-medium rounded-md flex items-center justify-between group ${activePriceType === 'retail' ? 'bg-blue-50 text-blue-700' : 'text-gray-700 hover:bg-gray-50'}`}
                                    >
                                        <div className="flex items-center gap-2"><Tag size={14} className={activePriceType === 'retail' ? 'text-blue-500' : 'text-gray-400'}/> Harga Eceran</div>
                                        {activePriceType === 'retail' && <Check size={14} />}
                                    </button>
                                    <button 
                                        type="button"
                                        onClick={() => { setActivePriceType('kingFano'); setShowPriceMenu(false); }}
                                        className={`w-full text-left px-3 py-2 text-xs font-medium rounded-md flex items-center justify-between group ${activePriceType === 'kingFano' ? 'bg-purple-50 text-purple-700' : 'text-gray-700 hover:bg-gray-50'}`}
                                    >
                                        <div className="flex items-center gap-2"><Crown size={14} className={activePriceType === 'kingFano' ? 'text-purple-500' : 'text-gray-400'}/> King Fano</div>
                                        {activePriceType === 'kingFano' && <Check size={14} />}
                                    </button>
                                </div>
                            </div>
                        )}
                        ...
                        <div className="relative">
                            ...
                            <input 
                                type="number" 
                                min="0" 
                                value={activePriceType === 'retail' ? formData.price : (formData.kingFanoPrice || 0)} 
                                onChange={handlePriceChange} 
                                className={`w-full pl-8 pr-4 py-2 bg-white border rounded-lg focus:ring-2 outline-none text-sm font-bold text-gray-900 transition-all ${activePriceType === 'retail' ? 'border-blue-300 focus:ring-blue-500' : 'border-purple-300 focus:ring-purple-500'}`} 
                                placeholder="0" 
                            />
                        </div>
                    </div>
            ...
            <div className="bg-blue-50/50 p-4 rounded-xl border border-blue-100">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    <div>
                        ...
                        <input type="number" name="initialStock" min="0" value={formData.initialStock} onChange={handleChange} className="w-full px-3 py-2 bg-white border border-gray-300 rounded-lg outline-none text-sm text-center" />
                    </div>
                    <div>
                        <label className="block text-[10px] font-bold text-green-600 uppercase mb-1">Penambahan</label>
                        <input type="number" name="qtyIn" min="0" value={formData.qtyIn} onChange={handleChange} className="w-full px-3 py-2 bg-white border border-green-300 rounded-lg outline-none text-sm text-center text-green-700 font-bold" />
                    </div>
                    <div>
                        <label className="block text-[10px] font-bold text-red-600 uppercase mb-1">Pengurangan</label>
                        <input type="number" name="qtyOut" min="0" value={formData.qtyOut} onChange={handleChange} className="w-full px-3 py-2 bg-white border border-red-300 rounded-lg outline-none text-sm text-center text-red-700 font-bold" />
                    </div>
                    <div>
