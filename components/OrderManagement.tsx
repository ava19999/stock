...
export const OrderManagement: React.FC<OrderManagementProps> = ({ orders = [], onUpdateStatus, onProcessReturn, onRefresh }) => {
  ...
  // 2. Fetch Scan Logs saat Tab Scan Aktif
  useEffect(() => {
      if (activeTab === 'scan') {
          loadScanLogs();
  ...
  // 3. Auto-Select "Siap Kirim" di Scan Logs
  useEffect(() => {
    if (activeTab === 'scan') {
    ...
    reader.onload = async (evt) => {
        try {
            ...
            if (updates.length > 0) {
                ...
                if (success) {
                    alert(`Berhasil memproses ${updates.length} data.`);
                    await loadScanLogs();
                } else {
  ...
  const getOrderDetails = (order: Order) => {
      ...
      try {
          ...
          if (resiMatch && resiMatch[1]) { resiText = resiMatch[1]; cleanName = cleanName.replace(/\s*\(Resi:.*?\)/, ''); }
          const shopMatch = cleanName.match(/\(Toko: (.*?)\)/);
          if (shopMatch && shopMatch[1]) { shopName = shopMatch[1]; cleanName = cleanName.replace(/\s*\(Toko:.*?\)/, ''); }
          const viaMatch = cleanName.match(/\(Via: (.*?)\)/);
          if (viaMatch && viaMatch[1]) { ecommerce = viaMatch[1]; cleanName = cleanName.replace(/\s*\(Via:.*?\)/, ''); }
  ...
  const handleProcessReturn = async () => {
      ...
      try {
        ...
        // Gunakan timestamp order untuk tanggal pemesanan
        ...
        for (const item of itemsToReturnData) {
            ...
            if (realItem) {
                await updateInventory({ ...realItem, quantity: realItem.quantity }, {
                    type: 'in', qty: item.cartQuantity, ecommerce: ecommerce, resiTempo: combinedResiShop, customer: cleanName, price: hargaSatuan, isReturn: true 
                });
            }
            ...
            const returData: ReturRecord = {
                ...
                resi: resiText, toko: shopName, ecommerce: ecommerce, customer: cleanName,
                part_number: item.partNumber, nama_barang: item.name, quantity: item.cartQuantity,
                harga_satuan: hargaSatuan, harga_total: totalRetur, tanggal_retur: new Date().toISOString(),
                status: statusRetur, keterangan: 'Retur Barang'
            };
  ...
  // --- B. Untuk Order & Retur (Tab lainnya) ---
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-200 min-h-[80vh] flex flex-col overflow-hidden relative">
      ...
      {/* MODAL RETURN & EDIT NOTE (HANYA UNTUK TAB ORDERS/HISTORY) */}
      {isNoteModalOpen && editingNoteData && (
          <div className="absolute inset-0 z-[60] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in">
              <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden flex flex-col">
                  ...
                  <div className="p-4">
                      <textarea className="w-full p-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-purple-200 outline-none text-sm min-h-[100px]" placeholder="Masukkan alasan atau catatan..." value={noteText} onChange={(e) => setNoteText(e.target.value)} />
                  </div>
      ...
      <div className="flex border-b border-gray-100 bg-gray-50/50">
          {[
              ...
              { id: 'scan', label: 'Scan Resi', icon: ScanBarcode, count: 0, color: 'text-gray-800' }, 
      ...
      {activeTab === 'scan' ? (
        ...
        <>
                ...
                <div className="flex flex-col md:flex-row gap-3">
                    ...
                    <div className="flex gap-2">
                        <div className="relative flex shadow-sm rounded-lg flex-1 md:flex-none">
                            <button onClick={() => fileInputRef.current?.click()} disabled={isUploading} className={`flex items-center gap-2 px-3 py-2.5 text-sm font-bold border rounded-l-lg transition-colors flex-1 md:w-32 justify-center ${getMarketplaceColor(selectedMarketplace)}`}>
                                {isUploading ? <Loader2 size={16} className="animate-spin"/> : <FileSpreadsheet size={16} />}
                                {isUploading ? 'Loading...' : selectedMarketplace}
                            </button>
                            <button onClick={() => setShowMarketplacePopup(!showMarketplacePopup)} className="px-2 bg-white border-y border-r border-gray-200 rounded-r-lg hover:bg-gray-50 flex items-center justify-center">
                            ...
                            {showMarketplacePopup && (
                                <div className="absolute top-full left-0 mt-1 w-48 bg-white rounded-xl shadow-xl border border-gray-100 z-50 animate-in fade-in zoom-in-95 overflow-hidden">
                                    {MARKETPLACES.map((mp) => (
                                        <button key={mp} onClick={() => { setSelectedMarketplace(mp); setShowMarketplacePopup(false); barcodeInputRef.current?.focus(); }} className={`w-full text-left px-4 py-3 text-sm flex items-center justify-between hover:bg-gray-50 border-b border-gray-50 last:border-0 ${selectedMarketplace === mp ? 'text-blue-600 font-bold bg-blue-50' : 'text-gray-700'}`}>
                                            {mp}
                        ...
                        <select value={selectedStore} onChange={(e) => { setSelectedStore(e.target.value); barcodeInputRef.current?.focus(); }} className="bg-gray-50 border border-gray-200 text-gray-700 text-sm font-bold rounded-lg focus:ring-blue-500 focus:border-blue-500 p-2.5">
                    ...
                    {/* GROUP 3: Kamera & Button Kirim */}
                    <div className="flex gap-2">
                        <button onClick={() => cameraInputRef.current?.click()} disabled={analyzing} className="text-gray-700 bg-white border border-gray-200 hover:bg-gray-50 font-bold rounded-lg text-sm px-4 py-2.5 flex items-center justify-center gap-2">
                        ...
                        {readyToSendCount > 0 && (
                            <button onClick={handleProcessKirim} disabled={isProcessingShipment} className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold px-4 py-2 rounded-lg shadow-sm transition-all animate-in fade-in slide-in-from-right-5">
                                {isProcessingShipment ? <Loader2 size={14} className="animate-spin"/> : <Send size={14} />}
                                Proses ({readyToSendCount})
                            </button>
                        )}
                    </div>
                </div>
            ...
            {/* TABEL SCAN RESI (DENGAN PAGINATION SLICE) */}
                            ...
                            {scanCurrentItems.length === 0 ? (
                                <tr>
                                    <td colSpan={11} className="p-8 text-center text-gray-400">
                                        <div className="flex flex-col items-center gap-2"><ScanBarcode size={32} className="opacity-20"/><p>Belum ada resi yang di-scan hari ini</p></div>
                                    </td>
                                </tr>
                            ) : (
                                        ...
                                        <tr key={log.id || idx} className={`transition-colors ${isSold ? 'bg-gray-50 opacity-60' : (isSelected ? 'bg-blue-50' : 'hover:bg-gray-50')}`}>
                                            ...
                                            <td className="px-4 py-3 text-center whitespace-nowrap">
                                                {isSold ? (
                                                    ...
                                                    <span className="inline-flex items-center gap-1 text-amber-600 font-bold bg-amber-50 px-2 py-0.5 rounded-full border border-amber-100 text-[10px]"><Edit2 size={10}/> Data Kosong</span>
                                                )}
                                            </td>
                                        </tr>
                                    );
        ...
        <>
            ...
            {/* PAGINATION FOOTER (ORDER/RETUR TAB) */}
            <div className="px-4 py-3 bg-white border-t border-gray-200 flex items-center justify-between text-xs text-gray-500">
                <div>Menampilkan {startIndex + 1}-{Math.min(startIndex + itemsPerPage, totalItems)} dari {totalItems} data</div>
                <div className="flex items-center gap-2">
                    <button onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1} className="p-1 rounded hover:bg-gray-100 disabled:opacity-30 disabled:cursor-not-allowed"><ChevronLeft size={16}/></button>
                    <span className="font-bold text-gray-900">Halaman {currentPage}</span>
                    <button onClick={() => setCurrentPage(p => Math.min(totalPages, p + 1))} disabled={currentPage >= totalPages} className="p-1 rounded hover:bg-gray-100 disabled:opacity-30 disabled:cursor-not-allowed"><ChevronRight size={16}/></button>
                </div>
            </div>
        </>
      )}
