...
export const OrderManagement: React.FC<OrderManagementProps> = ({ orders = [], onUpdateStatus, onProcessReturn, onRefresh }) => {
  // GANTI TAB 'processing' MENJADI 'terjual'
  const [activeTab, setActiveTab] = useState<'pending' | 'terjual' | 'history'>('pending');
  ...
  useEffect(() => {
      fetchReturRecords().then(records => { setReturDbRecords(records); });
  }, [activeTab, orders]);
      ...
      try {
          ...
          if (success) {
              setReturDbRecords(prev => prev.map(item => {
                  if ((item.id && item.id.toString() === editingNoteData.id) || item.resi === editingNoteData.resi) return { ...item, keterangan: noteText };
                  return item;
              }));
  ...
  const handleProcessReturn = async () => {
      ...
      try {
        ...
        for (const item of itemsToReturnData) {
            ...
            if (realItem) {
                await updateInventory({ ...realItem, quantity: realItem.quantity }, {
                    type: 'in', qty: item.cartQuantity, ecommerce: ecommerce, 
                    resiTempo: combinedResiShop, customer: cleanName, price: hargaSatuan, isReturn: true 
                });
            }
            ...
            await addReturTransaction({
                tanggal_pemesanan: orderDate, resi: resiText, toko: shopName, ecommerce: ecommerce, customer: cleanName,
                part_number: item.partNumber, nama_barang: item.name, quantity: item.cartQuantity,
                harga_satuan: hargaSatuan, harga_total: totalRetur, tanggal_retur: new Date().toISOString(), status: statusRetur, keterangan: 'Retur Barang'
            });
        }
        ...
        if (remainingItems.length === 0) {
            ...
            await saveOrder({
                id: `${selectedOrderForReturn.id}-RET`, customerName: `${selectedOrderForReturn.customerName} (RETUR)`,
            ...
            // UPDATE: Gunakan 'terjual' untuk sisa barang
            await updateOrderData(selectedOrderForReturn.id, remainingItems, remainingTotal, 'terjual');
        }
      ...
      } catch (error) { console.error("Error processing return:", error); } finally { setIsLoading(false); }
  };
  ...
  const filteredData = useMemo(() => {
    if (activeTab === 'history') {
        return returDbRecords.filter(r => {
            ...
            return ((r.resi && r.resi.toLowerCase().includes(lower)) || (r.customer && r.customer.toLowerCase().includes(lower)) || (r.nama_barang && r.nama_barang.toLowerCase().includes(lower)));
        }).sort((a, b) => new Date(b.tanggal_retur).getTime() - new Date(a.tanggal_retur).getTime());
    } else {
        return safeOrders.filter(o => {
            if (!o) return false;
            if (activeTab === 'pending') return o.status === 'pending';
            // FILTER: Mendukung 'terjual' dan 'processing' (legacy)
            if (activeTab === 'terjual') return o.status === 'terjual' || o.status === 'processing';
            return false;
        }).filter(o => {
            if (activeTab === 'terjual' && searchTerm.trim() !== '') {
                const lowerSearch = searchTerm.toLowerCase();
                return (o.id.toLowerCase().includes(lowerSearch) || o.customerName.toLowerCase().includes(lowerSearch) || o.items.some(item => item.name.toLowerCase().includes(lowerSearch)));
            }
            return true;
  ...
  const getStatusColor = (status: OrderStatus) => {
    switch (status) {
      ...
      case 'terjual': return 'bg-blue-100 text-blue-700 border-blue-200'; // WARNA TERJUAL
  ...
  const formatDate = (ts: number | string) => { try { const date = new Date(ts || Date.now()); return { date: date.toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' }), time: date.toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' }) }; } catch (e) { return { date: '-', time: '-' }; } };
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-200 min-h-[80vh] flex flex-col overflow-hidden relative">
      ...
      {isNoteModalOpen && editingNoteData && (
          <div className="absolute inset-0 z-[60] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in">
              <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden flex flex-col">
                  ...
                  <div className="p-4"><textarea className="w-full p-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-purple-200 outline-none text-sm min-h-[100px]" placeholder="Masukkan alasan atau catatan..." value={noteText} onChange={(e) => setNoteText(e.target.value)}/></div>
                  <div className="p-3 border-t bg-gray-50 flex justify-end gap-2"><button onClick={() => setIsNoteModalOpen(false)} className="px-3 py-1.5 text-xs font-bold text-gray-600 hover:bg-gray-200 rounded-lg">Batal</button><button onClick={handleSaveNote} disabled={isSavingNote} className="px-4 py-1.5 text-xs font-bold bg-purple-600 text-white hover:bg-purple-700 rounded-lg shadow flex items-center gap-2">{isSavingNote ? <Loader size={14} className="animate-spin"/> : <Save size={14}/>} Simpan</button></div>
              </div>
          </div>
      )}
      ...
      {isReturnModalOpen && selectedOrderForReturn && (
          <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in">
              <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90%]">
                  <div className="bg-orange-50 px-4 py-3 border-b border-orange-100 flex justify-between items-center"><h3 className="text-base font-bold text-orange-800 flex items-center gap-2"><RotateCcw size={18}/> Retur Barang</h3><button onClick={() => setIsReturnModalOpen(false)}><X size={18} className="text-orange-400 hover:text-orange-600"/></button></div>
                  <div className="p-4 overflow-y-auto text-sm"><div className="space-y-2">{selectedOrderForReturn.items.map((item) => (<div key={item.id} className="flex items-center justify-between p-2 border border-gray-200 rounded-lg hover:border-orange-200"><div className="flex-1"><div className="font-bold text-gray-800 text-xs">{item.name}</div><div className="text-[10px] text-gray-500 font-mono">{item.partNumber}</div></div><div className="flex items-center gap-2 bg-gray-50 p-1 rounded-lg border border-gray-200"><button onClick={() => setReturnQuantities(prev => ({...prev, [item.id]: Math.max(0, (prev[item.id] || 0) - 1)}))} className="w-6 h-6 flex items-center justify-center bg-white rounded shadow-sm hover:bg-red-50 text-gray-600 font-bold">-</button><div className="w-6 text-center font-bold text-sm text-gray-800">{returnQuantities[item.id] || 0}</div><button onClick={() => setReturnQuantities(prev => ({...prev, [item.id]: Math.min(item.cartQuantity || 0, (prev[item.id] || 0) + 1)}))} className="w-6 h-6 flex items-center justify-center bg-white rounded shadow-sm hover:bg-green-50 text-gray-600 font-bold">+</button></div></div>))}</div></div>
                  <div className="p-3 border-t bg-gray-50 flex justify-end gap-2"><button onClick={() => setIsReturnModalOpen(false)} className="px-3 py-1.5 text-xs font-bold text-gray-600 hover:bg-gray-200 rounded-lg">Batal</button><button onClick={handleProcessReturn} disabled={isLoading} className="px-4 py-1.5 text-xs font-bold bg-orange-600 text-white hover:bg-orange-700 rounded-lg shadow flex items-center gap-2">{isLoading ? <Loader size={14} className="animate-spin"/> : <Save size={14}/>} Proses</button></div>
              </div>
          </div>
      )}
      ...
      <div className="px-4 py-3 border-b border-gray-100 bg-white flex justify-between items-center">
        <div><h2 className="text-lg font-bold text-gray-900 flex items-center gap-2"><ClipboardList className="text-purple-600" size={20} /> Manajemen Pesanan</h2></div>
      </div>
      ...
      <div className="flex border-b border-gray-100 bg-gray-50/50">
          {[
              ...
              { id: 'terjual', label: 'Terjual', icon: Package, count: 0, color: 'text-blue-600' }, 
      ...
      {/* SEARCH BAR (Compact) */}
      {(activeTab === 'terjual' || activeTab === 'history') && (
          <div className="px-4 py-2 bg-gray-50 border-b border-gray-100">
              <div className="relative max-w-sm"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={14} /><input type="text" placeholder="Cari Resi / Nama..." className="w-full pl-9 pr-8 py-1.5 bg-white border border-gray-200 rounded-md text-xs focus:ring-1 focus:ring-purple-200 focus:border-purple-400 outline-none" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}/>{searchTerm && (<button onClick={() => setSearchTerm('')} className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600"><X size={12} /></button>)}</div>
          </div>
      )}
            ...
            <table className="w-full text-left border-collapse">
                <thead className="bg-gray-50 text-[10px] font-bold text-gray-600 uppercase tracking-wider border-b border-gray-200">
                    <tr>
                        ...
                        <th className="px-3 py-2 w-32">Resi / Tempo</th>
                        ...
                        {activeTab === 'history' ? (<><th className="px-3 py-2 w-24 bg-red-50/50 text-red-600 border-l border-red-100">Tgl Retur</th><th className="px-3 py-2 text-center w-24 bg-red-50/50 text-red-600">Status</th></>) : (<th className="px-3 py-2 text-center w-24">Status</th>)}
                        <th className="px-3 py-2 text-center w-32">{activeTab === 'history' ? 'Ket' : 'Aksi'}</th>
                    </tr>
                </thead>
                <tbody className="divide-y divide-gray-100 text-xs">
                    {activeTab === 'history' ? (
                        currentItems.length === 0 ? (<tr><td colSpan={13} className="p-8 text-center text-gray-400"><ClipboardList size={32} className="opacity-20 mx-auto mb-2" /><p>Belum ada data</p></td></tr>) : ((currentItems as ReturRecord[]).map((retur) => {
                            const dtOrder = formatDate(retur.tanggal_pemesanan || ''); const dtRetur = formatDate(retur.tanggal_retur);
                            return (<tr key={`retur-${retur.id}`} className="hover:bg-red-50/20 transition-colors">
                                <td className="px-3 py-2 align-top border-r border-gray-100"><div className="font-bold text-gray-900">{dtOrder.date}</div></td>
                                <td className="px-3 py-2 align-top font-mono text-[10px] text-gray-600"><div className="flex flex-col gap-1"><span className="font-bold text-blue-700 bg-blue-50 px-1.5 py-0.5 rounded border border-blue-100 w-fit">{retur.resi || '-'}</span>{retur.toko && <span className="uppercase text-gray-500 bg-gray-100 px-1 py-0.5 rounded w-fit">{retur.toko}</span>}</div></td>
                                <td className="px-3 py-2 align-top">{retur.ecommerce ? <span className="px-1.5 py-0.5 bg-orange-50 text-orange-700 text-[9px] font-bold rounded border border-orange-100">{retur.ecommerce}</span> : '-'}</td>
                                ...
                                <td className="px-3 py-2 align-top border-l border-red-100 bg-red-50/10"><div className="font-bold text-red-700 text-[10px]">{dtRetur.date}</div></td>
                                <td className="px-3 py-2 align-top text-center bg-red-50/10"><span className={`px-1.5 py-0.5 rounded text-[9px] font-bold border uppercase ${retur.status === 'Full Retur' ? 'bg-red-100 text-red-700 border-red-200' : 'bg-orange-100 text-orange-700 border-orange-200'}`}>{retur.status || 'Retur'}</span></td>
                                <td className="px-3 py-2 align-top"><div className="flex items-start justify-between gap-1 group/edit"><div className="text-[10px] text-gray-600 italic truncate max-w-[100px]">{retur.keterangan || '-'}</div><button onClick={() => openNoteModal(retur)} className="text-blue-500 hover:bg-blue-50 p-1 rounded opacity-0 group-hover/edit:opacity-100"><Edit3 size={12} /></button></div></td>
                            </tr>)
                        }))
                    ) : (
                        /* --- TAB PENDING & TERJUAL --- */
                        currentItems.length === 0 ? (<tr><td colSpan={11} className="p-8 text-center text-gray-400"><ClipboardList size={32} className="opacity-20 mx-auto mb-2" /><p>{searchTerm ? 'Tidak ditemukan' : 'Tidak ada data'}</p></td></tr>) : ((currentItems as Order[]).map(order => {
                                ...
                                return items.map((item, index) => {
                                    ...
                                    <tr key={`${order.id}-${index}`} className="hover:bg-blue-50/10 transition-colors group">
                                        {index === 0 && (<><td rowSpan={items.length} className="px-3 py-2 align-top border-r border-gray-100 bg-white group-hover:bg-blue-50/10"><div className="font-bold text-gray-900">{dt.date}</div><div className="text-[9px] text-gray-400 font-mono">{dt.time}</div></td><td rowSpan={items.length} className="px-3 py-2 align-top border-r border-gray-100 font-mono text-[10px] bg-white group-hover:bg-blue-50/10"><div className="flex flex-col gap-1"><span className={`px-1.5 py-0.5 rounded w-fit font-bold border ${isResi ? 'bg-blue-50 text-blue-700 border-blue-100' : 'text-gray-500 bg-gray-50 border-gray-200'}`}>{resiText}</span>{shopName !== '-' && <div className="flex items-center gap-1 text-gray-600 bg-gray-100 px-1.5 py-0.5 rounded w-fit border border-gray-200"><Store size={8} /><span className="uppercase truncate max-w-[80px]">{shopName}</span></div>}</div></td><td rowSpan={items.length} className="px-3 py-2 align-top border-r border-gray-100 bg-white group-hover:bg-blue-50/10">{ecommerce !== '-' ? <div className="px-1.5 py-0.5 rounded bg-orange-50 text-orange-700 border border-orange-100 w-fit text-[9px] font-bold">{ecommerce}</div> : <span className="text-gray-300">-</span>}</td><td rowSpan={items.length} className="px-3 py-2 align-top border-r border-gray-100 font-medium text-gray-900 bg-white group-hover:bg-blue-50/10 truncate max-w-[120px]" title={cleanName}>{cleanName}</td></>)}
                                        ...
                                        {index === 0 && (<><td rowSpan={items.length} className="px-3 py-2 align-top text-center border-l border-gray-100 bg-white group-hover:bg-blue-50/10"><div className={`inline-block px-2 py-0.5 rounded text-[9px] font-extrabold border uppercase mb-1 shadow-sm ${getStatusColor(order.status)}`}>{getStatusLabel(order.status)}</div><div className="text-[10px] font-extrabold text-purple-700">{formatRupiah(order.totalAmount || 0)}</div></td><td rowSpan={items.length} className="px-3 py-2 align-top text-center border-l border-gray-100 bg-white group-hover:bg-blue-50/10"><div className="flex flex-col gap-1 items-center">{order.status === 'pending' && (<><button onClick={() => onUpdateStatus(order.id, 'terjual')} className="w-full py-1 bg-purple-600 text-white text-[9px] font-bold rounded hover:bg-purple-700 shadow-sm flex items-center justify-center gap-1">Proses</button><button onClick={() => onUpdateStatus(order.id, 'cancelled')} className="w-full py-1 bg-white border border-gray-300 text-gray-600 text-[9px] font-bold rounded hover:bg-red-50 hover:text-red-600">Tolak</button></>)}{(order.status === 'terjual' || order.status === 'processing') && (<button onClick={() => openReturnModal(order)} className="w-full py-1 bg-orange-50 border border-orange-200 text-orange-600 text-[9px] font-bold rounded hover:bg-orange-100 flex items-center justify-center gap-1">Retur</button>)}</div></td></>)}
                                    </tr>);
                                });
                            }))
                    )}
      ...
      <div className="px-4 py-3 bg-white border-t border-gray-200 flex items-center justify-between text-xs text-gray-500">
          <div>Menampilkan {startIndex + 1}-{Math.min(startIndex + itemsPerPage, totalItems)} dari {totalItems} data</div>
          <div className="flex items-center gap-2"><button onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1} className="p-1 rounded hover:bg-gray-100 disabled:opacity-30 disabled:cursor-not-allowed"><ChevronLeft size={16}/></button><span className="font-bold text-gray-900">Halaman {currentPage}</span><button onClick={() => setCurrentPage(p => Math.min(totalPages, p + 1))} disabled={currentPage >= totalPages} className="p-1 rounded hover:bg-gray-100 disabled:opacity-30 disabled:cursor-not-allowed"><ChevronRight size={16}/></button></div>
      </div>
    </div>
  );
};