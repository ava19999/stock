...
import { ScanBarcode, Upload, Loader2, Save, CheckCircle, FileSpreadsheet, Keyboard, Search, X } from 'lucide-react';
import { formatRupiah, parseCSV } from '../utils'; 
...
export const ScanResiView: React.FC<ScanResiProps> = ({ onSave, onSaveBulk, isProcessing }) => {
  ...
  }, [activeTab, result]); // Re-focus setelah result ditutup/disimpan

  // --- LOGIC HANDLER SCANNER (ENTER) ---
  ...
  const processScannedCode = (code: string) => {
    // 1. Coba cari di data Excel yang sudah diupload (parsedOrders)
    const foundInExcel = parsedOrders.find(o => 
        o.resi.toLowerCase() === code.toLowerCase() || 
        o.resi.toLowerCase().includes(code.toLowerCase()) // Toleransi partial match
    );
    ...
    if (foundInExcel) {
        ...
        // 2. Jika tidak ada di Excel, buat data manual (User harus isi sisanya)
        setResult({
            resi: code,
            date: new Date().toLocaleDateString('id-ID'),
            ecommerce: 'Manual Scan',
            customerName: '', // Kosongkan agar user isi
            items: [] // Kosongkan
        });
    }
  };

  // --- LOGIC SELEKSI CHECKBOX ---
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 space-y-6 pb-20">
                          ...
                          <table className="w-full text-left text-sm">
                              <thead className="bg-gray-100 text-gray-600 font-bold text-xs uppercase sticky top-0 z-10 shadow-sm">
                                  <tr>
                                      <th className="p-4 w-12 text-center">
                                          <input type="checkbox" className="w-4 h-4 rounded border-gray-300 text-green-600 focus:ring-green-500 cursor-pointer" checked={parsedOrders.length > 0 && selectedIndices.size === parsedOrders.length} onChange={toggleSelectAll} />
                                      </th>
                              ...
                              <tbody className="divide-y divide-gray-100 bg-white">
                                  {parsedOrders.map((o, i) => {
                                      ...
                                      return (
                                        <tr key={i} className={`transition-colors group cursor-pointer ${isSelected ? 'bg-green-50/60' : 'hover:bg-gray-50'}`} onClick={() => toggleSelectRow(i)}>
                                            <td className="p-4 align-top text-center" onClick={(e) => e.stopPropagation()}><input type="checkbox" className="w-4 h-4 rounded border-gray-300 text-green-600 focus:ring-green-500 cursor-pointer" checked={isSelected} onChange={() => toggleSelectRow(i)} /></td>
      ...
      {/* === TAB 2: SCANNER FISIK (INPUT FIELD) === */}
      {activeTab === 'barcode' && (
        <>
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-8 animate-in fade-in slide-in-from-right-4 relative overflow-hidden text-center">
            ...
            <div className="max-w-md mx-auto relative">
                ...
                <input 
                    ...
                    placeholder="Klik disini & Scan Resi..." 
            ...
            <div className="mt-4 flex flex-col items-center justify-center gap-2">
                <p className="text-sm text-gray-500">Pastikan kursor aktif di kotak input di atas sebelum menggunakan alat scanner.</p>
                {parsedOrders.length > 0 && (
                    <span className="bg-green-100 text-green-700 px-3 py-1 rounded-full text-xs font-bold border border-green-200 flex items-center gap-1">
                        <FileSpreadsheet size={12}/> {parsedOrders.length} Data Excel Terhubung
                    </span>
                )}
            </div>
        </div>
