...
import { 
  ...
  fetchHistory, addHistoryLog, fetchChatSessions, saveChatSession,
  addBarangMasuk, addBarangKeluar, updateOrderData 
...
const AppContent: React.FC = () => {
  ...
  const handleSaveItem = async (data: InventoryFormData) => {
      ...
      if (editItem) { if (await updateInventory(updatedItem)) { showToast('Update berhasil!'); refreshData(); } } else { if (items.some(i => i.partNumber === data.partNumber)) { showToast('Part Number sudah ada!', 'error'); setLoading(false); return; } if (await addInventory(data)) { showToast('Tersimpan!'); refreshData(); } }
      setIsEditing(false); setEditItem(null); setLoading(false);
  };
  const handleUpdateBanner = async (base64: string) => { const bannerData: any = { partNumber: BANNER_PART_NUMBER, name: 'SYSTEM BANNER PROMO', application: 'DO NOT DELETE', brand: 'SYS', price: 0, costPrice: 0, ecommerce: '', quantity: 0, initialStock: 0, qtyIn: 0, qtyOut: 0, shelf: 'SYSTEM', imageUrl: base64 }; if (await (bannerUrl ? updateInventory(bannerData) : addInventory(bannerData))) { setBannerUrl(base64); showToast('Banner diperbarui!'); } else { showToast('Gagal update banner', 'error'); } };
  ...
  const handleProcessReturn = async (orderId: string, returnedItems: { itemId: string, qty: number }[]) => {
      ...
      for (const retur of returnedItems) {
          const itemInOrder = order.items.find(i => i.id === retur.itemId); if (!itemInOrder) continue;
          const currentItem = await getItemByPartNumber(itemInOrder.partNumber);
          if (currentItem) {
              const restoreQty = retur.qty; const newQuantity = currentItem.quantity + restoreQty;
      ...
      const newItems = order.items.map(item => { const returInfo = returnedItems.find(r => r.itemId === item.id); if (returInfo) { const newQty = item.cartQuantity - returInfo.qty; return { ...item, cartQuantity: newQty }; } return item; }).filter(item => item.cartQuantity > 0); 
      ...
      if (await updateOrderData(orderId, newItems, newTotal, newStatus)) { showToast('Retur berhasil diproses & Stok kembali!'); await refreshData(); } else { showToast('Gagal update data pesanan', 'error'); }
      setLoading(false);
  };
  ...
  const handleUpdateStatus = async (orderId: string, newStatus: OrderStatus) => {
      ...
      // UPDATE: Tangani status 'terjual' (menggantikan 'processing')
      if (order.status === 'pending' && (newStatus === 'terjual' || newStatus === 'processing')) {
          const finalStatus = 'terjual'; // Pastikan yang disimpan ke DB adalah 'terjual'
          if (await updateOrderStatusService(orderId, finalStatus)) { 
              for (const orderItem of order.items) {
                  const currentItem = await getItemByPartNumber(orderItem.partNumber);
                  if (currentItem) {
                      const qtySold = orderItem.cartQuantity; const newQuantity = Math.max(0, currentItem.quantity - qtySold);
              ...
              showToast('Pesanan TERJUAL, stok berkurang.'); refreshData();
          }
      }
          ...
          if (await updateOrderStatusService(orderId, newStatus, updateTime)) {
              if (order.status !== 'pending') {
                  for (const orderItem of order.items) {
                      const currentItem = await getItemByPartNumber(orderItem.partNumber);
                      if (currentItem) {
                          const restoreQty = orderItem.cartQuantity; const newQuantity = currentItem.quantity + restoreQty;
                          const itemToUpdate = { ...currentItem, qtyOut: Math.max(0, (currentItem.qtyOut || 0) - restoreQty), quantity: newQuantity, lastUpdated: Date.now() };
                          await updateInventory(itemToUpdate);
      ...
      } else { if (await updateOrderStatusService(orderId, newStatus, updateTime)) { refreshData(); } }
  };
  ...
  const handleSaveScannedOrder = async (data: ResiAnalysisResult | any) => {
    ...
    const matchedCartItems: CartItem[] = []; const unmatchedItems: string[] = [];
    for (const scannedItem of data.items) {
        const foundItem = items.find(i => i.name.toLowerCase().includes(scannedItem.name.toLowerCase()) || (i.partNumber && i.partNumber.toLowerCase() === scannedItem.name.toLowerCase()));
        if (foundItem) { matchedCartItems.push({ ...foundItem, cartQuantity: scannedItem.qty || 1, customPrice: foundItem.price }); } else { unmatchedItems.push(`${scannedItem.name}`); }
    }
    if (matchedCartItems.length === 0) { showToast(`Gagal! ${unmatchedItems.length} barang tidak ditemukan di sistem.`, 'error'); setLoading(false); return; }
    const totalAmount = matchedCartItems.reduce((sum, item) => sum + (item.price * item.cartQuantity), 0);
    
    // UPDATE: Default status untuk scan resi adalah 'terjual'
    const newOrder: Order = { id: generateId(), customerName: finalCustomerName, items: matchedCartItems, totalAmount: totalAmount, status: 'terjual', timestamp: Date.now() };
    ...
    if (await saveOrder(newOrder)) {
        ...
        for (const item of matchedCartItems) {
            ...
            if (currentItem) {
                const qtySold = item.cartQuantity; const newQuantity = Math.max(0, currentItem.quantity - qtySold);
                ...
                await addBarangKeluar({ tanggal: today, kodeToko: 'SCAN', tempo: 'MJM', ecommerce: data.ecommerce || 'MANUAL', customer: data.customerName || 'GUEST', partNumber: currentItem.partNumber, name: currentItem.name, brand: currentItem.brand, application: currentItem.application, rak: currentItem.shelf, stockAhir: newQuantity, qtyKeluar: qtySold, hargaSatuan: item.price, hargaTotal: item.price * qtySold, resi: data.resi || '-' });
            }
        }
        if (unmatchedItems.length > 0) showToast(`Sukses Sebagian! ${unmatchedItems.length} SKU dilewati.`, 'error'); else showToast('Scan Resi Berhasil! Stok Terupdate.', 'success');
        if (activeView === 'scan') setActiveView('orders'); await refreshData();
    } else { showToast('Gagal menyimpan pesanan', 'error'); }
    setLoading(false);
  };

  const handleBulkSave = async (ordersList: any[]) => { setLoading(true); let successCount = 0; for (const orderData of ordersList) { try { await handleSaveScannedOrder(orderData); successCount++; } catch (e) { console.error("Gagal import:", orderData.resi); } } showToast(`Proses Selesai! ${successCount} pesanan diproses.`, 'success'); setActiveView('orders'); setLoading(false); };
  ...
  if (!isAuthenticated) { return ( <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50 flex items-center justify-center p-4 font-sans"> {toast && <Toast message={toast.msg} type={toast.type} onClose={() => setToast(null)} />} <div className="bg-white/80 backdrop-blur-xl p-8 rounded-3xl shadow-2xl w-full max-w-md border border-white/50 relative overflow-hidden"> <div className="absolute top-0 right-0 w-32 h-32 bg-blue-400/10 rounded-full blur-3xl -mr-10 -mt-10"></div> <div className="absolute bottom-0 left-0 w-24 h-24 bg-purple-400/10 rounded-full blur-2xl -ml-5 -mb-5"></div> <div className="relative z-10"> <div className="flex justify-center mb-6"><div className="bg-white p-4 rounded-2xl shadow-lg ring-1 ring-gray-100"><Car size={40} className="text-blue-600" strokeWidth={1.5} /></div></div> <div className="text-center mb-8"><h1 className="text-4xl font-extrabold text-gray-900 tracking-tight mb-1">BJW</h1><p className="text-gray-700 text-lg font-bold uppercase tracking-wider mb-1">Autopart</p><p className="text-gray-500 text-sm">Sukucadang Mobil</p></div> <form onSubmit={handleGlobalLogin} className="space-y-5"> <div className="space-y-1.5"><label className="text-xs font-semibold text-gray-500 uppercase tracking-wider ml-1">Identitas</label><div className="relative group"><User className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-blue-500 transition-colors" size={20} /><input type="text" value={loginName} onChange={(e) => setLoginName(e.target.value)} className="w-full pl-12 pr-4 py-3.5 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 outline-none transition-all font-medium text-gray-800 placeholder:text-gray-400" placeholder="Nama Anda..." /></div></div> <div className="space-y-1.5"><label className="text-xs font-semibold text-gray-500 uppercase tracking-wider ml-1">Kode Akses <span className="text-gray-300 font-normal">(Opsional)</span></label><div className="relative group"><KeyRound className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-blue-500 transition-colors" size={20} /><input type="password" value={loginPass} onChange={(e) => setLoginPass(e.target.value)} className="w-full pl-12 pr-4 py-3.5 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 outline-none transition-all font-medium text-gray-800 placeholder:text-gray-400" placeholder="Password Admin" /></div></div> <button type="submit" className="w-full bg-gray-900 hover:bg-black text-white font-bold py-4 rounded-xl shadow-lg hover:shadow-xl transition-all active:scale-[0.98] flex items-center justify-center gap-2 group"><span>Masuk Aplikasi</span><ArrowRight size={18} className="group-hover:translate-x-1 transition-transform" /></button> </form> <div className="mt-6 pt-6 border-t border-gray-100 flex flex-col items-center gap-3"><button onClick={() => loginAsCustomer('Tamu')} className="text-sm font-medium text-gray-500 hover:text-blue-600 transition-colors py-2 px-4 hover:bg-blue-50 rounded-lg w-full text-center">Masuk sebagai Tamu</button></div> </div> </div> </div> ); }
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col font-sans text-gray-900">
      ...
      <div className="bg-white border-b px-4 py-3 flex justify-between items-center sticky top-0 z-50 shadow-sm backdrop-blur-md bg-white/90">
          <div className="flex items-center gap-3 cursor-pointer group" onClick={() => setActiveView(isAdmin ? 'inventory' : 'shop')}>
              ...
              <div><div className="font-bold leading-none text-gray-900 text-lg">BJW</div><div className="text-[10px] font-bold text-gray-600 leading-none mt-0.5">Autopart</div><div className="text-[9px] text-gray-400 leading-none">Sukucadang Mobil</div><div className={`text-[9px] font-bold mt-1 px-1.5 py-0.5 rounded-md inline-block ${isAdmin ? 'bg-purple-50 text-purple-700' : 'bg-blue-50 text-blue-700'}`}>{isAdmin ? 'ADMIN ACCESS' : 'STORE FRONT'}</div></div>
          </div>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {isEditing && isAdmin && (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm animate-in fade-in"><div className="w-full max-w-4xl max-h-[90vh] overflow-y-auto rounded-2xl shadow-2xl"><ItemForm initialData={editItem || undefined} onSubmit={handleSaveItem} onCancel={() => { setIsEditing(false); setEditItem(null); }} /></div></div>)}
      </div>
        ...
        <div className={`grid ${isAdmin ? 'grid-cols-5' : 'grid-cols-3'} h-16`}>
            {isAdmin ? (
                ...
                <>
                    ...
                    <button onClick={()=>setActiveView('orders')} className={`relative flex flex-col items-center justify-center gap-1 ${activeView==='orders'?'text-blue-600':'text-gray-400 hover:text-gray-600'}`}><div className="relative"><ClipboardList size={22} className={activeView==='orders'?'fill-blue-100':''} />{myPendingOrdersCount > 0 && <span className="absolute -top-1 -right-1 w-2.5 h-2.5 bg-orange-500 rounded-full border border-white"></span>}</div><span className="text-[10px] font-medium">Pesanan</span></button>
