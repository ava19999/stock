...
import { InventoryItem, InventoryFormData, CartItem, Order, ChatSession, Message, OrderStatus, StockHistory } from './types'; // Tambahkan StockHistory
...
import { Home, MessageSquare, Package, ShieldCheck, User, CheckCircle, XCircle, ClipboardList, LogOut, ArrowRight, CloudLightning, RefreshCw, KeyRound } from 'lucide-react';
...
const HISTORY_STORAGE_KEY = 'stockmaster_history_db_v1'; // Key baru untuk history
...
const AppContent: React.FC = () => {
  ...
  const [history, setHistory] = useState<StockHistory[]>([]); // State History
  ...
  // --- LOGIKA UPDATE STOK DAN HISTORY ---
  const handleSaveItem = async (data: InventoryFormData) => {
      ...
      if (editItem) {
          // MODE EDIT: Hitung selisih stok
          const oldQty = Number(editItem.quantity) || 0;
          const diff = quantityNum - oldQty;
          ...
          if (diff !== 0) {
              addHistory({
                  ...
                  name: data.name,
                  type: diff > 0 ? 'in' : 'out',
                  quantity: Math.abs(diff), // Simpan angka positif (misal: 1)
                  previousStock: oldQty,
                  currentStock: quantityNum,
                  timestamp: Date.now(),
                  reason: diff > 0 ? 'Restock Manual' : 'Koreksi Stok Manual'
              });
          }
          ...
          // MODE TAMBAH BARU: Semua stok dianggap masuk
          if (items.some(i => i.partNumber === data.partNumber)) { 
              showToast('Part Number ada!', 'error'); setLoading(false); return; 
          }

          addHistory({
              id: generateId(),
              itemId: data.partNumber, // ID sementara sebelum sync
              partNumber: data.partNumber,
              name: data.name,
              type: 'in',
      ...
      if(confirm('Hapus dari Google Sheet?')) {
          ...
          if (itemToDelete) {
              ...
              if (success) {
                  ...
                  // Catat history penghapusan (optional)
  ...
  const doCheckout = (name: string) => {
      ...
      // Catat History Keluar untuk setiap item
      cart.forEach(cartItem => {
          ...
          addHistory({
              ...
              currentStock: currentQty - cartItem.cartQuantity,
              timestamp: Date.now(),
  ...
  const handleUpdateStatus = (orderId: string, newStatus: OrderStatus) => {
      ...
      if (newStatus === 'cancelled' && order.status !== 'cancelled') {
          // Restore Stock Logic & Catat History
          setItems(prevItems => {
              const updatedItems = [...prevItems];
              order.items.forEach(orderItem => {
                  const invItem = updatedItems.find(i => i.id === orderItem.id);
                  if (invItem) {
                      const oldQty = invItem.quantity;
                      invItem.quantity += orderItem.cartQuantity;
                      ...
                      addHistory({
                          ...
                          timestamp: Date.now(),
                          reason: `Cancel Order #${orderId.slice(0,6)}`
                      });
                  }
              });
            ...
            <div className="bg-white/80 backdrop-blur-xl p-8 rounded-3xl shadow-2xl w-full max-w-md border border-white/50 relative overflow-hidden">
                ...
                <div className="relative z-10">
                    ...
                    <form onSubmit={handleGlobalLogin} className="space-y-5">
                        ...
                        <button type="submit" className="w-full bg-gray-900 hover:bg-black text-white font-bold py-4 rounded-xl shadow-lg hover:shadow-xl transition-all active:scale-[0.98] flex items-center justify-center gap-2 group">
                            <span>Masuk Aplikasi</span>
                            <ArrowRight size={18} className="group-hover:translate-x-1 transition-transform" />
                        </button>
                    </form>
                    <div className="mt-6 pt-6 border-t border-gray-100 flex flex-col items-center gap-3">
                        <button onClick={() => loginAsCustomer('Tamu')} className="text-sm font-medium text-gray-500 hover:text-blue-600 transition-colors py-2 px-4 hover:bg-blue-50 rounded-lg w-full text-center">Masuk sebagai Tamu</button>
                    </div>
                </div>
            </div>
        </div>
